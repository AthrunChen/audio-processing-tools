...
def augment_data(y, sr, n_augment = 0, allow_speedandpitch = True, allow_pitch = True,
    allow_speed = True, allow_dyn = True, allow_noise = False, allow_timeshift = False, tab=""):
...
#        print(tab+"augment_data: ",i+1,"of",n_augment)
        ...
        if (allow_speedandpitch) and random_onoff():   
            length_change = np.random.uniform(low=0.75,high=1.3)
...
#            print(tab+"    speed_change = ",speed_change)
            y_mod = y_mod * np.random.uniform(0.8, 1.2)
        ...
        if (allow_dyn) and random_onoff():  
            dyn_change = np.random.uniform(low=0.85,high=1.4)  # change amplitude
...
#            print(tab+"    timeshift_fac = ",timeshift_fac)
        ...
        # speed_change = np.random.uniform(low=1.2, high=1.6)
        # #            print(tab+"    speed_change = ",speed_change)
        # tmp = librosa.effects.time_stretch(y_mod, speed_change)
        # minlen = min(y.shape[0], tmp.shape[0])  # keep same length as original;
        # y_mod *= 0  # pad with zeros
        ...
        # speed_rate = np.random.uniform(0.68, 1.0)
        # y_mod = cv2.resize(y_mod, (1, int(len(y_mod) * speed_rate))).squeeze()
        # mods.append(y_mod)

        # scope = np.random.uniform(0.45, 1.4)
        # y_mod = y_mod*scope
        mods.append(y_mod)
    ...
    for path, pathname, filenames in os.walk(in_dir):
        for filename in filenames:
            if filename.endswith('wav'):
                ...
                mods = augment_data(y, sr, n_augment=1, allow_timeshift=False)
                for i in range(len(mods)-1):
                    # filename_no_ext = os.path.splitext(os.path.join(dstfile,filename))[0]
                    filename_no_ext = os.path.splitext(os.path.join(in_dir, filename))[0]
                    ext = os.path.splitext(os.path.join(path, filename))[1]
                    # outfile = filename_no_ext+str(i+1)+"000"+ext
                    outfile = filename_no_ext + ext
                    print(outfile)
                    mods[i+1] = np.int16(mods[i+1] * 32768)
                    scipy.io.wavfile.write(outfile, sr, mods[i+1])
                    # librosa.output.write_wav(outfile,mods[i+1],sr)scipy.io.wavfile.write(outfile, sr, mods[i+1])
...
if __name__ == "__main__":
    auto_aug(r"C:\Users\Administrator\Downloads\chxb\datacleaning_output")
    # auto_aug(sys.argv[1])
